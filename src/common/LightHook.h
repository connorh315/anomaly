...
static const unsigned char PREFIXES[] = {0xF0, 0xF2, 0xF3, 0x2E, 0x36, 0x3E,
                                         0x26, 0x64, 0x65, 0x66, 0x67};
static const unsigned char OP1_MODRM[] = {0x62, 0x63, 0x69, 0x6B, 0xC0, 0xC1, 0xC4, 0xC5, 0xC6,
                                          0xC7, 0xD0, 0xD1, 0xD2, 0xD3, 0xF6, 0xF7, 0xFE, 0xFF};
static const unsigned char OP1_IMM8[] = {0x6A, 0x6B, 0x80, 0x82, 0x83, 0xA8, 0xC0,
 ...
 */
static int FindByte(const unsigned char* buffer, const unsigned long long maxLength,
...
static int GetInstructionSize(const void* address) {
    ...
    if (*b == 0x0F) {
        ...
        if (*b == 0x38 || *b == 0x3A) {
            ...
            if (HOOK_R == 8)
                offset += 4;
            else if ((HOOK_R == 7 && HOOK_C < 4) || *b == 0xA4 || *b == 0xC2 ||
                     (*b > 0xC3 && *b <= 0xC6) || *b == 0xBA || *b == 0xAC)
                offset++;

            if (FindByte(OP2_MODRM, sizeof(OP2_MODRM), *b) ||
                (HOOK_R != 3 && HOOK_R > 0 && HOOK_R < 7) || *b >= 0xD0 ||
                (HOOK_R == 7 && HOOK_C != 7) || HOOK_R == 9 || HOOK_R == 0xB ||
                (HOOK_R == 0xC && HOOK_C < 8) || (HOOK_R == 0 && HOOK_C < 4))
                ParseModRM(&b, addressPrefix);
        }
    } else {
        if ((HOOK_R == 0xE && HOOK_C < 8) || (HOOK_R == 0xB && HOOK_C < 8) || HOOK_R == 7 ||
            (HOOK_R < 4 && (HOOK_C == 4 || HOOK_C == 0xC)) || (*b == 0xF6 && !(*(b + 1) & 48)) ||
            FindByte(OP1_IMM8, sizeof(OP1_IMM8), *b))
            offset++;
        else if (*b == 0xC2 || *b == 0xCA)
            offset += 2;
        else if (*b == 0xC8)
            offset += 3;
        else if ((HOOK_R < 4 && (HOOK_C == 5 || HOOK_C == 0xD)) || (HOOK_R == 0xB && HOOK_C >= 8) ||
                 (*b == 0xF7 && !(*(b + 1) & 48)) || FindByte(OP1_IMM32, sizeof(OP1_IMM32), *b)) {
        ...
        if (FindByte(OP1_MODRM, sizeof(OP1_MODRM), *b) ||
            (HOOK_R < 4 && (HOOK_C < 4 || (HOOK_C >= 8 && HOOK_C < 0xC))) || HOOK_R == 8 ||
            (HOOK_R == 0xD && HOOK_C >= 8))
            ParseModRM(&b, addressPrefix);
    }
...
static const unsigned char JUMP_CODE[] = {0xFF, 0x25, 0x00, 0x00, 0x00, 0x00, 0xFF,
                                          0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

/**
 * \brief Prepare hook information structure and backup original function bytes that will be used
 * for the trampoline
...
/**
 * \brief Change memory protection flags in usermode, disable write protection bit in cr0 in kernel
 * mode
 * \param address Memory location
 * \param size Size in bytes (will likely be rounded to one page)
 * \param protection Use PROTECTION_READ_WRITE_EXECUTE to make region RWX, otherwise platform
 * specific
...
static int EnableHook(HookInformation* information) {
    ...
    int bufferSize = sizeof(JUMP_CODE) + information->BytesToCopy;
    if (bufferSize % 0x4000 != 0) {
        bufferSize = (bufferSize + 0x10000 - 1) & ~(0x10000 - 1);
    }
    ...
    //unsigned long long originalProtection = PlatformProtect(
    //    information->OriginalFunction, information->BytesToCopy, PROTECTION_READ_WRITE_EXECUTE);
...
static int DisableHook(HookInformation* information) {
    ...
    unsigned long long originalProtection = PlatformProtect(
        information->OriginalFunction, information->BytesToCopy, PROTECTION_READ_WRITE_EXECUTE);
    MemoryCopy(information->OriginalFunction, information->OriginalBuffer,
